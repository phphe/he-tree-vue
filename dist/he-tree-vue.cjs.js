...
function makeTreeDraggable(treeEl, options = {}) {
  ...
  const {
    ...
    getMovedOrClonedElement: (directTriggerElement, store) => {
      ...
      const el = hp.findParent(store.triggerElement, el => hp.hasClass(el, options.branchClass), {
        withSelf: true
      });
      return el;
    },
    ...
    beforeMove: (store, dhOptions) => {
      ...
      const info = {
        ...
        closestNode: () => {
          ...
          //
          let found;
          const t = hp.binarySearch(nodes, node => hp.getOffset(node).y - movingNodeOf.y, {
